...
def format_message(msg, msg_len, filename=None):
    if not filename:  # text
        msg = MAGIC_NUMBER + msg_len + (0).to_bytes(1, 'big') + msg
    else:
        filename = filename.encode('utf-8')
        filename_len = len(filename).to_bytes(1, 'big')
        msg = MAGIC_NUMBER + msg_len + filename_len + filename + msg

    return msg


def encode_message(host_data, msg, bits):
    ...
    print("Message size: {:,} bytes".format(len(msg)))
    print("Maximum size: {:,} bytes".format(host_data.size // divisor))

    check_message_space(host_data.size // divisor, len(msg))
    ...
    msg[:len(msg)] = list(msg)

    host_data[:divisor * len(msg)] &= 256 - 2 ** bits  # clear last bit(s)
